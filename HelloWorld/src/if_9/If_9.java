...
//		もし score が 80点以上なら「〇〇さんは□□点です。たいへんよくできました。」とコンソールに出力して下さい。
		...
		String name = "志々目さんは";
//		scoreに0から100までのランダムな数値を設定
		Random random = new Random();
				int score = random.nextInt(101);

//		if文を使って、scoreの値に応じて異なるメッセージを出力
		if (score < 40){
			System.out.println(name + score + "点です。もう少し頑張りましょう。");

		}else if (score >=40 && score < 60){
		System.out.println(name + score + "点です。もう少しでした。");

		}else if (score >=60 && score < 80){
			System.out.println(name + score + "点です。よくできました。");

		}else {
			System.out.println(name + score + "点です。たいへんよくできました。");
		}
...
//		④もし変数 isTired が 0 であれば、変数 study を利用して「カリキュラムを続けます。」と表示する。
		...
		int isTired = 0;
		String study = "カリキュラム";
//		「isTired」が0なら、「私はとても元気です。」と表示され、1なら「私は疲れています。」と表示
		if (isTired == 0){
